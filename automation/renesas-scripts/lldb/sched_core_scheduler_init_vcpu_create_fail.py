# Ensure scheduler core initialization forces crash if vcpu_create() for idle_domain fails
...
def VcpuCreate(frame):
    dbg.force_return(frame, "0x0")

def SchedulerInit(frame):
    dbg.install_entry_hook("vcpu_create", VcpuCreate)
