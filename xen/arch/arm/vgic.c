...
int vcpu_vgic_init(struct vcpu *v)
{
    ...
    {
        check_errs();
        if ( vgic_has_directVSGI(v->domain) || true)
            /*
    ...
    check_errs();
    if ( vgic_has_directVLPI(v->domain) && gicv3_its_host_has_its())
    {
        check_errs();
    ...
    check_errs();
    memset(STACK_SHIFT, 0, sizeof(STACK_SHIFT));
    return 0;
}
