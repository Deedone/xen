...
struct page_info *lpi_allocate_proptable(void)
{
    void *table;
    int order;
    ...
    //TODO fix
    // table = _xmalloc(lpi_data.max_host_lpi_ids, SZ_4K);
    order = get_order_from_bytes(max(lpi_data.max_host_lpi_ids, (unsigned long)SZ_64K));
