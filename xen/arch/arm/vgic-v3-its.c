                                    ...
                                    unsigned int evid_bits)
{
    ...
    if ( is_iommu_enabled(its->d) )
    {
        ...
        {
            mfn = maddr_to_mfn(host_doorbell_addr);
            flush_flags = 0;
            ret = iommu_map(its->d, _dfn(PFN_DOWN(its->doorbell_address)),
            ...
            ret = iommu_iotlb_flush(its->d,
                                    _dfn(PFN_DOWN(its->doorbell_address)),
                                    1, flush_flags);
            if ( ret < 0 )
                return ret;
        }
