...
static uint32_t dt_msi_map_id(uint32_t id_in)
{
    ...
    printk(XENLOG_ERR "Calling dt_map_id\n");
    rc = dt_map_id(bridge->dt_node, id_in, "msi-map", "msi-map-mask", NULL,
                   &id_out);
    printk(XENLOG_ERR "rc is %d id_out is %d\n", rc, id_out);
    if ( rc )
        return rc;
...
static bool vgic_its_check_event_id(struct vgic_its *its,
                                    struct its_device *device, u32 event_id)
{
                                    ...
                                    u64 *its_cmd)
{
    ...
    if ( device )
        ...
        device =
            vgic_its_alloc_device(its, guest_devid, itt_addr, num_eventid_bits);
...
static struct vgic_register_region its_registers[] = {
    ...
    REGISTER_ITS_DESC(GITS_IIDR, vgic_mmio_read_its_iidr, its_mmio_write_wi, 4,
                      VGIC_ACCESS_32bit),
    REGISTER_ITS_DESC(GITS_TYPER, vgic_mmio_read_its_typer, its_mmio_write_wi,
    ...
    REGISTER_ITS_DESC(GITS_CWRITER, vgic_mmio_read_its_cwriter,
                      ...
                      VGIC_ACCESS_64bit | VGIC_ACCESS_32bit),
    REGISTER_ITS_DESC(GITS_CREADR, vgic_mmio_read_its_creadr, its_mmio_write_wi,
                      8, VGIC_ACCESS_64bit | VGIC_ACCESS_32bit),
