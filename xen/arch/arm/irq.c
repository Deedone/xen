...
static void evacuate_irq(int irq, unsigned int from)
{
     ...
     * to be completely safe we must reroute it to a new, definitely online,
     * CPU. Set the new affinity to a mask of currently online CPUs so the GIC
     * driver picks one of them at random.
     */
