...
static int __init its_vpe_init(struct its_vpe *vpe)
{
    ...
    // vpe->vpendtable = ioremap_nocache(virt_to_maddr(_xzalloc(SZ_64K, SZ_64K)), SZ_64K);
    if ( gic_has_v4_1_extension() )
...
int vgic_v4_its_vpe_init(struct vcpu *vcpu)
{
     ...
     */
     ...
     //     int i;
     //     uint32_t *buf;
     //     printk("TABLES BASER 2 %lx\n", hw_its->tables[2].val);
     //     printk("TABLES BASER 2 base %p\n", hw_its->tables[2].base);
     //     printk("TABLES BASER 2 size %x\n", hw_its->tables[2].table_size);
         
     //     buf = hw_its->tables[2].base;
         
     //     for (i = 0; i < hw_its->tables[2].table_size / 4; i++ ) {
     //        // for (i = 0; i < 100; i++ ) {
     //         printk("TABLE[%d] = %x\n", i, buf[i]);
     //     }
     // }
...
static void its_make_vpe_resident(struct its_vpe *vpe, unsigned int cpu)
{
    ...
    // printk("MAKE VPE RESIDENT UPDATED!!!!!!\n");
...
static int its_send_cmd_vmovp(struct its_vpe *vpe)
{
    ...
    if ( !its_list_map )
    {
        ...
        cmd[1] = (uint64_t)vpeid << 32;
        cmd[2] = encode_rdbase(hw_its, vpe->col_idx, 0x0) << 16;
        cmd[3] = 0x00;

        if ( hw_its->is_v4_1 )
            encode_vmovp_v4_1(cmd, vpe->vpe_db_lpi);

        // printk("VMOVP %d CPU %d\n", vpeid, vpe->col_idx);
...
static int gicv4_vpe_set_affinity(struct vcpu *vcpu)
{
    ...
    // printk("VPE %d SET AFFINITY FROM %u TO %u\n", vpe->vpe_id, from, to);
    vpe->col_idx = to;
