...
static int vf_init_bars(struct pci_dev *vf_pdev)
{
    ...
    vf_idx = vf_pdev->sbdf.sbdf - (pf_pdev->sbdf.sbdf + sriov->offset);
    if ( vf_idx < 0 )
        return -EINVAL;

    if ( sriov->num_vfs > 1 && !sriov->stride )
        return -EINVAL;

    if ( sriov->stride )
    {
        if ( vf_idx % sriov->stride )
            return -EINVAL;
        vf_idx /= sriov->stride;
    }
    ...
    for ( i = 0; i < PCI_SRIOV_NUM_BARS; i++ )
    {
        ...
        /*
         * The upper half of a 64-bit BAR holds no address or size of its own;
         * the handlers reach the lower half through its type alone.
         */
        if ( pf_bar->type == VPCI_BAR_MEM64_HI )
        {
        ...
        bar->guest_addr   = is_hardware_domain(vf_pdev->domain) ? bar->addr : 0;
...
int vpci_vf_init_header(struct pci_dev *vf_pdev)
{
    ...
    ctrl = pci_conf_read16(pf_pdev->sbdf,
                           pf_pdev->vpci->sriov->pos + PCI_SRIOV_CTRL);
         ...
         pf_pdev->domain != vf_pdev->domain )
    {
        struct vpci_bar *bars = vf_pdev->vpci->header.bars;
        /*
         * A VF's Vendor and Device ID registers read as 0xffff. The guest has
         * no PF to synthesise them from, so take the vendor from the PF and
         * the device from its SR-IOV capability.
         */
...
/*
 * Accesses to a marked BDF are terminated until 100ms after VF Enable; the
 * first one past that adds the VF, which control_write() cannot do itself.
 */
                                   ...
                                   uint32_t val, void *data)
{
    ...
    /* VF BARs decode only while both VF Enable and VF MSE are set. */
    bool decoding = enabled && (control & PCI_SRIOV_CTRL_MSE);
    bool new_decoding = new_enabled && (val & PCI_SRIOV_CTRL_MSE);
    int rc;

    ASSERT(!pdev->info.is_virtfn);

    if ( new_enabled == enabled && new_decoding == decoding )
    {
        pci_conf_write16(pdev->sbdf, reg, val);
        return;
    }

    /* Unmap while the VF pdevs are still around for vpci_modify_bars(). */
    if ( decoding && !new_decoding )
        map_vfs(pdev, 0);
...
static int cf_check init_sriov(struct pci_dev *pdev)
{
    ...
    pdev->vpci->sriov = xvzalloc(struct vpci_sriov);
    if ( !pdev->vpci->sriov )
        return -ENOMEM;
